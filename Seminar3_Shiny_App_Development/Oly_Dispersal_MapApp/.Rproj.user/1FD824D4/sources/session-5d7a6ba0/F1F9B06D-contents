...
##### Sample 10 tracks from selected Sites & Behaviors, then save as new Rdata
sites <- c("FB", "PB", "SamB", "SimB", "KH", "UR")
behaviors <- c("none", "onto", "photo")

loop_list <- list(); i <- 0
for(site_id in sites) {
  for(behav in behaviors) {
    if(site_id %in% c("FB", "PB", "SamB", "SimB", "KH")) {
      releases <- 477
...
load(here("data", "ssm_grid_sfEdge.Rdata"))

ssm_poly <- ssm_grid_sfEdge |>
  st_cast("POLYGON")

save(ssm_poly, file = here("data", "ssm_poly.Rdata"))
